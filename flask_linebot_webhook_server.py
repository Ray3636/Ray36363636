...
# 1. 先到Line Developer Console，把Channel Secret & Channel Access Token複製起來
# 2. 把這兩個密文存到環境變數內:搜尋<環境變數>，新增兩個環境變數，並且把值貼上去
...
# 設計一個 callback 的路由，提供給Line官方後臺去呼叫
# 也就是所謂的呼叫Webhook Server
...
# 根據不同的使用者事件(event)，用不同的方式回影
# eg. MessageEvent 代表使用者單純傳訊息的事件
# TextMessageContent 代表使用者傳書的訊息內容是文字
